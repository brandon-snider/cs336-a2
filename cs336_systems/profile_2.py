...
Usage:
- Forward only:
    uv run nsys profile -o forward python -m cs336_systems.profile_2 --size xl --seq-len 512 --mode forward
- Forward + backward:
    uv run nsys profile -o forward_backward python -m cs336_systems.profile_2 --size xl --seq-len 512 --mode forward_backward
- Train (forward + backward + optimizer step):
    uv run nsys profile -o train python -m cs336_systems.profile_2 --size medium --mode train
- Also annotate scaled_dot_product_attention:
    uv run nsys profile -o train_sdpa python -m cs336_systems.profile_2 --size medium --mode train --annotate-sdpa
"""
...
def _sdpa_annotated(Q: torch.Tensor, K: torch.Tensor, V: torch.Tensor, mask=None):  # noqa: N802
    with nvtx.range("scaled dot product attention"):
        with nvtx.range("compute scores"):
            d_k = K.shape[-1]
            scores = torch.einsum("...qd,...kd->...qk", Q, K) / torch.sqrt(
